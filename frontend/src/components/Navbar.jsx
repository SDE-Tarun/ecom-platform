		...
		<nav className="py-3">
			<div className={`container position-relative d-flex justify-content-between align-items-center flex-column ${showBorder ? 'showLine' : ''}`}>
				<main className="col-12 d-flex justify-content-between align-items-center">
					...
					{/* Mobile Menu */}
					<ul
						className={`position-fixed p-0 top-0 end-0 z-1 d-flex flex-column ${showMenu ? "show" : ""}`}
					>
						<li
					...
					<div className="right d-flex align-items-center gap-1 gap-sm-3">
						<div className="icons d-flex align-items-center gap-1 gap-sm-3">
							{showSearchIcon && (
								<i
							...
							{user ? (
								<>
									<span className="fs-small c-gray">
										Hi👋, <span className="fw-semibold">{user.name?.split(" ")[0]}</span>
									</span>

									<i
										className="bx bx-log-out fs-little-big c-gray cursor"
